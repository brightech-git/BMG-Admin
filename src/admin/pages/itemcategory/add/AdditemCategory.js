...
import { useUploadCategory  ,useUpdateCategory } from "../../../hooks/itemCategory/useUploadCategory";
...
const ItemCategory = () => {
    ...
    // queries / mutations  
    const { data: bannersData, isLoading: bannersLoading } = useCategories();

    const banners = useMemo(() => bannersData || [], [bannersData]);

    const { items: itemNames = [] } = useItemNames();

    const uploadMutation = useUploadCategory();
    ...
    const allBanners = useMemo(() => {
        ...
        return banners.map((item) => item.item_name);
    }, [banners]);
        ...
        if (isEdit) {
            updateMutation.mutate(payload, {
                onSuccess: () => {
                    ...
                    setTimeout(() => {
                        navigate("/item-category/manage"); // go back to list (change if needed)
                    }, 700);
                },
                onError: (err) => {
            ...
            uploadMutation.mutate(payload, {
                onSuccess: () => {
                    ...
                    setTimeout(() => {
                        navigate("/item-category/manage"); // go back to list
                    }, 700);
                },
                onError: (err) => {
                ...
                <form onSubmit={handleSubmit} className="space-y-2">
                    ...
                    <div className="flex items-center justify-between gap-3 pt-2">
                        <div className="flex gap-2">
                            ...
                            <button
                                type="button"
                                onClick={() => navigate("/item-category/add")}
                                className="px-2 py-1.5 rounded-md border text-xs bg-white dark:bg-slate-700"
                                disabled={isUploading || isUpdating}
                            >
                                Cancel
